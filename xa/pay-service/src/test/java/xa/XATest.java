...
public class XATest {
    public static XADataSource createXADataSource() throws Exception {
        ...
        dataSource.setURL("jdbc:mysql://192.168.1.241:33078/test?characterEncoding\\=UTF-8");
...
//            xaResource.start(xid1, XAResource.TMJOIN);// 全局id和分支id都一样，则使用 TMJOIN
            xaCons[0].getConnection().createStatement().executeUpdate("insert  into `tb_order`(`state`) values ('mutiProcess');");
            xaResource.end(xid1, XAResource.TMSUCCESS);
        } else {
            XAResource xaResource = xaCons[1].getXAResource();
//            xaResource.start(xid1, XAResource.TMNOFLAGS);// 全局id一样，新的分支id的，则使用 TMNOFLAGS
            xaResource.start(xid1, XAResource.TMJOIN);// 全局id和分支id都一样，则使用 TMJOIN
            ResultSet resultSet = xaCons[1].getConnection().createStatement().executeQuery("SELECT * FROM  tb_order");
            while (resultSet.next()) {
...
//                System.out.println("无需恢复格式：" + id.getFormatId());
    ...
    public static void join() throws Exception {
        ...
        xaCons[0].getConnection().createStatement().executeUpdate("insert  into `tb_order`(`state`) values ('join');");
        ...
        if (!xaResource1.isSameRM(xaResource0)) {
            ...
            xaCons[1].getConnection().createStatement().executeUpdate("insert  into `tb_order`(`state`) values ('joinb');");
            xaResource1.end(xid1, XAResource.TMSUCCESS);
        } else {
            MyXid xid2 = new MyXid(100, new byte[]{0x01}, new byte[]{0x03});
            xaResource1.start(xid2, XAResource.TMNOFLAGS);
            xaCons[1].getConnection().createStatement().executeUpdate("insert  into `tb_order`(`state`) values ('joinc');");
    ...
    public static void reUseXaRes2() throws Exception {
        ...
        stmt.executeUpdate("insert  into `tb_order`(`state`) values ('reUseXaRes2');");
    ...
    public static void reUseXaRes() throws Exception {
        ...
        stmt.executeUpdate("insert  into `tb_order`(`state`) values ('reUseXaRes');");
        ...
        stmt.executeUpdate("insert  into `tb_order`(`state`) values ('reUseXaRes b');");
    ...
    public static void suspend() throws Exception {
        ...
        try {
            ...
            stmt.executeUpdate("insert  into `tb_order`(`state`) values ('suspend');");//受到 commit/rollback控制
            ...
            stmt.executeUpdate("insert  into `tb_order`(`state`) values ('suspend b');");//在end之后，不会受到 commit/rollback控制
...
//            xaRes.start(xid, XAResource.TMRESUME);//重新开启之前挂起的事务，注意如果当前连接已经建立了则不能进行，需要一个新的 xa 连接来进行
    ...
    public static void resume() throws Exception {
        ...
        try {
            ...
            stmt.executeUpdate("insert  into `tb_order`(`state`) values ('resume');')");//受到 commit/rollback控制
            xaRes.end(xid, XAResource.TMSUCCESS);//表示要结束，不能再开启
